                    ...
                    this.axios
                        ...
                        .then((resp)=>{
                            ...
                            if(data.result=="SUCCESS"){
                                ...
                                //  nested router of this route
                                
                                // guide for making vue files

                                this.$session.get("loginInfo").usrMnuAtrt.forEach(function (item) {
                                    console.log(item.mnu_nm);
                                    item.nodeList.forEach(function (item) {
                                       
                                        let purl = item.mnu_url.replace(".do", ".vue");
                                        //  let purlarr = purl.split("/");
                                        
                                        //  let menuName=purlarr[2].replace('.vue','');
                                         let vueFilePath='@/views'+purl;
                                         console.log('  메뉴명: '+item.mnu_nm+' || 파일경로 : '+vueFilePath)   
                                //         console.log('path : '+menuName + ', path: '+vueFilePath);
                                //         // vm.$root.$router.addRoute('dashboard',{path:path.replace(".vue",""),component:()=>import('@/views'+purl)});    
                                //         vm.$router.push('dashboard',
                                //             {path:menuName,component:()=>import(vueFilePath)}
                                //         );
