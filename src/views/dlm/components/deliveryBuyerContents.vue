    ...
    <table class="col">
      ...
      <tbody v-for="(item, index) in searchlist" :key="item">
        <tr>
          ...
          <td
            @click="
              detailChk(item.deliv_id, item.deliverStatus, item.deliv_wh_id)
            "
          >
            {{ item.sales_nm }}
          </td>
...
import dvbyModal from "../dvbyModal.vue";
...
export default {
  ...
  methods: {
    ...
    listsearch: function (page) {
      ...
      this.axios
        .post("/dlm/listDeliOrder.do", params)
        .then(function (response) {
          //console.log(response);
          vm.totalCnt = response.data.totalCntListDeliOrder;

          vm.searchlist = response.data.listShippingModel;
            console.log("리스트 불러온다")
    ...
    detailChk: function (deliv_id, deliverStatus, deliv_wh_id) {
      const md = openModal(dvbyModal, {
        deliv_id: deliv_id,
