<template>
    ...
    <div id="lnb_area">
        ...
        <div class="login">
            ...
            <template v-if="$session.get('loginInfo').loginId">
                <img
...
export default {
    ...
    methods: {
        logoutproc: function () {
            let loginId = this.loginInfo.loginId;
            this.$session.set("loginInfo", {});

            // 서버의 session 에 저장된 클라이언트의 정보도 invalidate 작업이 필요함
            // jsp 로 구현한 백단 코드는 invalidate 후 login 화면으로 redirect
            // 새 코드가 필요할 것 같음
            // ex)
            ...
            // this.axios.post('logout.do')
            //           .then(()=>{
            //                      alert('로그아웃되었습니다');
            //                      this.$router.push('/');
            //            })

            alert(loginId + " 로그 아웃 되었습니다.");
            this.$router.push("/");
        },
